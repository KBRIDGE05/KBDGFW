(()=>{
  ...
  function cargoInfo(){
    const pieces=Math.max(1,num("lclPieces")||1),length=num("lclLength"),width=num("lclWidth"),height=num("lclHeight");
    const cbm=num("lclManualCbm")||(length&&width&&height?length*width*height*pieces/1e6:0),kg=num("lclWeight");
  ...
  function populateCountries(preferredCountry="",preferredId=""){
    ...
    const countries=[...new Set(entries.map(e=>e.country).filter(Boolean))].sort((a,b)=>a.localeCompare(b,"en",{sensitivity:"base"}));
    country.innerHTML='<option value="">도착 국가를 선택하세요</option>'+countries.map(name=>`<option value="${esc(name)}">${esc(name)}</option>`).join("");
  ...
  function populateDestinations(preferredId=""){
    ...
    if(status)status.textContent=`${elements().region?.selectedOptions?.[0]?.textContent||"조회 권역"} · ${selectedCountry||"도착 국가 선택"} · ${PERIOD_LABEL} 기준`;
    renderResult();
  }
  async function load(){
